...
const { createViewboxAroundCenter } = require("../utils/geo.js");
...
function normalizeNearbyNamePlaces(candidates, query) {
  return candidates
    ...
    .filter((place) => place.matchScore >= 55)
    .sort((left, right) => {
...
async function handleNearbyNameQuery(query, anchor, anchorType, allowBroadFallback, signal) {
  const localCandidates = await searchNominatim(query, {
    signal,
    limit: 12,
    viewbox: createViewboxAroundCenter(anchor, LOCAL_NAME_RADIUS_METERS),
    bounded: true,
    anchor
  });
  ...
  return {
    ...
    anchor: anchorType,
    fallbackUsed: true,
    quality: broaderCandidates.length ? "broad" : "empty",
    message: broaderCandidates.length
      ? `Showing broader results for "${query}".`
      : `No broader matches found for "${query}".`,
    category: null,
    canBroadSearch: false,
    resultsKind: "candidates",
    results: broaderCandidates
  };
}
